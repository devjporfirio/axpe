...
const CategoryBannerVertical = dynamic(
  () => import("components/CategoryBannerVertical"),
  {
    loading: () => null,
    ssr: true,
  },
...
const CategorySection = dynamic(
  () => import("../src/components/CategorySection"),
  {
    loading: () => null,
    ssr: false,
  },
...
const ResponsiveHeroImage = dynamic(
  () => import("../src/components/ResponsiveHeroImage"),
  {
    loading: () => null,
    ssr: true,
  },
...
function Home({ heroItems, components }) {
  ...
  const [isLighthouse, setIsLighthouse] = useState(() => {
    ...
    if (typeof window !== "undefined") {
      return (
        window.isLighthouse ||
        localStorage.getItem("lighthouse-simulation") === "true"
      );
    }
    return false;
  });
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      try {
        const lighthouseSimulation = localStorage.getItem(
  ...
  const heroSettings = (totalItems) => ({
    ...
    slidesToShow: 1,
    slidesToScroll: 1,
    customPaging: (i) => <span>{i + 1 + " / " + totalItems}</span>,
  });
  ...
  const renderComponents = useCallback((type, component) => {
    switch (type) {
      ...
      case "category":
        return <CategoryBannerVertical categoryItems={component.items} />;
      case "exclusivity":
        return (
      ...
      case "buildingsGrid":
        return <CategorySection items={component.items} />;
      case "gallery":
        return (
          <LazyLoad placeholderHeight={{ mobile: "650px", desktop: "550px" }}>
  ...
  return (
    <>
      ...
      <Container id="home-container-section-banners">
        <h1 className="sr-only">Axpe | Imóveis especiais São Paulo</h1>
