export const checkChristiesLogo = (data, suggestions = false) => {
		...
		// Suggestion list
		if(isActive && suggestions && suggestions instanceof Object && suggestions.length > 0) {
			let suggestionBuildings = [];
			if(suggestions.includes('0')) {
				suggestionBuildings = (suggestions[0].includes('items')) ? suggestions[0].items : suggestions[0];
			} else {
